...
%  l_bound = [300 -90]; % für einheitliche Achsen 
%  u_bound = [600 -82];
...
for i=1:result.opt.maxGen
    ...
    for j = 1:result.opt.numObj
        for k = 1:result.opt.numObj-1
            % Don't plot diagonal
            if j~=k
                subplot(result.opt.numObj ,result.opt.numObj,row_ind+k)
                plot(ind(:,result.opt.numVar+j,i), ind(:,result.opt.numVar+k,i), 'o');
                % axis limits
                %axis([l_bound(j) u_bound(j) l_bound(k) u_bound(k)]), für einheitliche Achsen
                box on
...
% plot(ind(:,result.opt.numVar+1), ind(:,result.opt.numVar+2), 'o','MarkerSize',6, 'color', 'b');
% %--------Referenz------------
...
% %set(y1, 'Units', 'Normalized', 'FontSize', 12);
% 
% %hAxis = gca;
