...
function [Analyse] = Analyse_PMSM(handles)
%Analyse Diese Funktion fuehrt die Analyse einer PMSM durch.
%   In der Funktion Analyse_PMSM werden Kennfelder fuer die vorher
...
%   - keine Dynamik
%   - verketteter Fluss der PM konstant
...
if(i_f>=opt.i_1max)
    ...
    if(betr.omega_k_max>omega_k_max2)
        ...
        warning('max. Drehzahl begrenzt durch hohen verketteten Fluss des PM');
    end
end
...
betr.mot.omega_el_mesh = betr.mot.omega_k_mesh; % elektrische Winkelgeschwindigkeit
...
if(opt.P_vu==1)
    ...
    elseif(strcmp(opt.P_vu_Modell,'Abschaetzung ueber verketteten Fluss'))
        % Variante 2: Berechnung der Eisenverluste ueber verketteten Fluss
        % Quelle: [Skript Antriebsregelung fuer Elektrofahrzeuge, TUM EAL]
        % Hinweis: nicht empfohlen, da aufgrund der empirischen Faktoren sehr ungenau
        
        % Empirische Faktoren
        loss.vu.c_Fe1 = 1.5; % zwischen 1.5 und 1.6
        loss.vu.c_Fe2 = 1.5; % zwischen 1.5 und 1.6
        
        % Berechnen der Eisenverluste
...
%   Bound (Motor): -i_1max<=i_1d<=0 (fuer L_d<L_q) bzw. 0<=i1_d<=i_1max (fuer L_d>L_q), 0<=i_1q<=i_1max
%   Linear Inequality Constraint: keine
%   Linear Equality Constraint: keine
